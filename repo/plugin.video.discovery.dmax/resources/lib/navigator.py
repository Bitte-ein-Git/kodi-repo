...
def list_themes():
	...
	for item in [{'name': 'Alaska', 'slug': 'alaska'},{'name': 'Alltagshelden', 'slug': 'alltagshelden'},{'name': 'Auktion', 'slug': 'auction'},
		...
		add_views({'mode': 'list_series', 'link': AURA_SEARCH.format(item['slug']), 'marker': 'revise_Themes', 'phase': 'taxonomies'}, FETCH_UNO)
...
def list_alphabet(target, phase):
	...
	for block in clientHelper().track_content(target).get('blocks', []):
		if re.sub(r'([^\w\s]|_)', '', block.get('title', 'unknown').lower()).endswith(phase): # Entferne alles ausser Wörtern und Leerzeichen = sendungen; sendungen:
			for item in sorted(block.get('items', []), key=lambda fix: fix.get('firstLetter', 'a')[:1]):
				if item.get('pageType') == 'showpage' or item.get('type') == 'showpage' and item.get('firstLetter', '') in letters and item['firstLetter'] not in UNIKAT: # Nach Alphabet filtern
					...
					add_views({'mode': 'list_series', 'link': AURA_NORM, 'marker': 'revise_Alphabet', 'phase': 'sendungen', 'extra': item['firstLetter']}, FETCH_UNO)
...
def list_series(target, marker, phase, extra): # 'revise_Favorites'; 'revise_Series'; 'revise_Themes'; 'revise_Alphabet'; 'news_Series'; 'last_Chance'
	...
	COMBI_UNO, COMBI_TRE = list_aurora(target, marker, phase, extra)
	if marker in ['revise_Favorites', 'news_Series', 'last_Chance']:
		...
		for each_due in sorted(fetch_pages(COMBI_UNO), key=lambda asx: asx.get(content, serve)[:16], reverse=adjust):
	...
	if COMBI_TRE:
		...
		for xev in RESULT:
			...
			add_views({'mode': 'list_episodes', 'link': AURA_ITEMS.format(SeriesSlug2), 'marker': marker, 'phase': phase, 'show': SeriesTitle2}, create_entries(FETCH_UNO), True, context, operation)
	xbmcplugin.endOfDirectory(ADDON_HANDLE)
...
def list_episodes(target, marker, phase, tvshow): # 'news_Episodes'
	...
	if marker == 'news_Episodes':
		COMBI_UNO, COMBI_TRE = list_aurora(target, marker, phase, "")
		for each_due in sorted(fetch_pages(COMBI_UNO), key=lambda asx: asx.get('publishStart', '2026-01-01T00:01')[:16], reverse=True):
			ser_slug = each_due.get('auraTicket', {}).get('secCipher', '')
			ser_title = each_due.get('show', {}).get('title', '')
			if ser_slug not in ISOLATE: # Nur jeweils einen Teil einer Serie behalten
				ISOLATE.add(ser_slug)
		...
		combies = clientHelper().track_content(target, headers=DEFAULT_HEADERS)
		if combies is not None and combies.get('blocks', []) and len(combies['blocks']) > 0 and any(sonic.get('type') in ['sonicShowBlock', 'seoSonicShowBlock'] for sonic in combies.get('blocks', {})):
			taxo_table = next(filter(lambda tox: tox.get('type') == 'showHeaderBlock' and tox.get('show', {}).get('taxonomies', {}), combies.get('blocks', {})), None) # Suche nach den Taxonomies-Inhalten im 'show' Ordner (Genres)
			ser_slug, ser_title = combies.get('slug', {}), combies.get('title', {})
			for scraps in combies.get('blocks', []):
				if scraps.get('type') in ['sonicShowBlock', 'seoSonicShowBlock'] and scraps.get('items', {}):
					post_table = scraps.get('items', {})
					if courses == 0: post_table = sorted(post_table, key=lambda sea: (int(sea.get('seasonNumber', 1)), int(sea.get('episodeNumber', 1))), reverse=True)
					for each_due in post_table:
						if taxo_table: each_due['taxonomies'] = taxo_table['show']['taxonomies'] # Kopiere Taxonomies-Ordner in 2. Ebene zu 'each_due'
						COMBI_DUE.append({'SeriesSlug': ser_slug, 'SeriesTitle': ser_title, 'Contents': each_due})
	if COMBI_DUE:
		debug_MS("---------------------------------------------")
		...
		for number, items in enumerate(COMBI_DUE, 1):
			...
			add_views({'mode': 'play_video', 'url': items.get('Contents', {}).get('id')}, create_entries(items, 'COLLATE', number), False)
	else:
...
def list_aurora(target, marker, phase, extra):
	...
	if package:
		...
		for article in combies:
			if article is not None and article.get('blocks', []) and len(article['blocks']) > 0 and any(sonic.get('type') in ['sonicShowBlock', 'seoSonicShowBlock'] for sonic in article.get('blocks', {})):
				number, codes = article.get('Position', 0), article.get('NaviCode', 'unknown')
				taxo_table = next(filter(lambda sox: sox.get('type') == 'showHeaderBlock' and sox.get('show', {}).get('taxonomies', {}), article.get('blocks', {})), None) # Suche nach den Taxonomies-Inhalten im 'show' Ordner (Genres)
				for scraps in article['blocks']:
					if scraps.get('type') in ['sonicShowBlock', 'seoSonicShowBlock'] and scraps.get('items', {}):
						points = len(scraps.get('items', {}))
						for single in scraps.get('items', {}):
							if not str(single.get('videoDuration')).isdecimal() or codes == 'unknown': continue # get('videoDuration) für Collect // get('item).get('videoDuration) für Series
							if taxo_table: single['taxonomies'] = taxo_table['show']['taxonomies'] # Kopiere Taxonomies-Ordner in 2. Ebene zu 'scraps'
							single.pop('features', None); single.pop('contentDescriptors', None); single.pop('package', None)
							embrace = {**single, **{'auraTicket': {'secCount': number, 'secCipher': codes, 'secPoints': points}}}
							yield embrace
